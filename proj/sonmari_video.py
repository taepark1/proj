...
#최소화가능 그냥 한장만 찍는걸로 만들어도 충분함 opcv로 cap에 입력
def video_capture(cap, width, height, frame_queue, darknet_image_queue):
    ...
    while cap.isOpened():
        ...
        #이미지 파일을 신경망에서 인식할 수 있는 데이터형식으로 변환
...
def inference(cap, args, network, class_names, darknet_image_queue, detections_queue, fps_queue):
    ...
    while cap.isOpened():
        ...
        #프레임
        prev_time = time.time()
        #캡처한 이미지에 대한 detect 결과 가져옴 detections(?)=리스트 모야 알아보기results=model.predict()사용하기 detctions리스트 생성
        ...
        #동영상 출력부분에 박스만들어서 객채 이름 출력하기 ?? 디버그시 사용하는 듯 없어도될듯?
...
def drawing(cap, window, args, width, height, class_colors, frame_queue, detections_queue, fps_queue):
    ...
    while cap.isOpened():         
        ...
        #resized된 프레임 가져오기 = 이미지파일
        frame_resized = frame_queue.get()
        #detections 가져오기 = darknet결과값 리스트
...
# class colors = darknet.load_network에 서 가져옴                   
            label, image = darknet.draw_boxes(detections, frame_resized, class_colors)
            ##draw_boxes = 다중반환 (튜플)? 불가능 return값 image 밖에 없음          
