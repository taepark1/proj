...
form_class = uic.loadUiType("gpt1.ui")[0]
...
class MyApp(QMainWindow, form_class) :
    def __init__(self):
        ...
        self.answer_carasel.setPixmap(self.pixmap)  # 'icon' QLabel에 QPixmap 객체 설정
        #self.answe..setPixmap(self.pixmap)
        
        #self.pixmap.load("prog.png")
        #self.pixmap = self.pixmap.scaledToWidth(100)
        self.progress_bar_1.setPixmap(self.pixmap)
        self.progress_bar_2.setPixmap(self.pixmap)
        self.progress_bar_3.setPixmap(self.pixmap)
        #self.pixmap.load("o.png")
        #self.pixmap = self.pixmap.scaledToWidth(100)
        self.o_x.setPixmap(self.pixmap)


        self.next_button = self.findChild(QPushButton, 'next_button') #여기서 동작변동
        if self.next_button :
            self.next_button.clicked.connect(self.on_button_clicked)
        
        self.previous_button = self.findChild(QPushButton, 'previous_button') #여기서 동작변동
        if self.previous_button :
            self.previous_button.clicked.connect(self.on_button_clicked)
            ...
            for r in detections:
                ...
                for box in boxes:
                    ...
                    if label in list(one.keys()) :
                        ...
                        window.pixmap.load("start.png")
                        window.pixmap = window.pixmap.scaledToWidth(100)
                        window.answer_carasel.setPixmap(window.pixmap)
                        
                    if label == 'person':
                        annotator.box_label(b, one.get(label))
                        window.pixmap.load("prog.png")
                        window.pixmap = window.pixmap.scaledToWidth(100)
                        window.progress_bar_1.setPixmap(window.pixmap)
                        progress = 1
                    if label == 'cell phone':
                        annotator.box_label(b, one.get(label))
                        window.pixmap.load("prog.png")
                        window.pixmap = window.pixmap.scaledToWidth(100)
                        window.progress_bar_2.setPixmap(window.pixmap)
                        progress = 2
                    if label == 'book' :
                        annotator.box_label(b, one.get(label))
                        window.pixmap.load("prog.png")
                        window.pixmap = window.pixmap.scaledToWidth(100)
                        window.progress_bar_3.setPixmap(window.pixmap)
                        progress = 3
                    if progress == 3 :
                        window.pixmap.load("o.png")
                        window.pixmap = window.pixmap.scaledToWidth(100)
                        window.o_x.setPixmap(window.pixmap)
                        progress = 0
                        #result = label
