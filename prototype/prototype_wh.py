...
#model = YOLO('project\prototype\train\book\best.pt')
...
class Ui_Dialog(object):
    ...
    #----
        ...
        Thread(target=video_capture, args=(cap, frame_queue, darknet_image_queue, start_time)).start()
        Thread(target=inference, args=(cap, args, darknet_image_queue, detections_queue, fps_queue, start_time)).start()
        Thread(target=drawing, args=(cap, self, args, frame_queue, detections_queue, fps_queue, start_time)).start()
...
def video_capture(cap, frame_queue, darknet_image_queue, start_time):
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
...
def inference(cap, args, darknet_image_queue, detections_queue, fps_queue, start_time):
    while cap.isOpened():
...
def drawing(cap, window, args, frame_queue, detections_queue, fps_queue, start_time):
    random.seed(3)
...
#논리   
        ...
        if elapsed_time < 3:
            cv2.putText(frame_resized, f"Starting in {3 - int(elapsed_time)} seconds", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        else:
